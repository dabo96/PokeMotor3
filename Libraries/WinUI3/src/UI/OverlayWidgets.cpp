...
bool BeginModal(const std::string &id, const std::string &title, uint32_t iconCodepoint,
  ...
  // brief 35 Part E: presence conduce el enter (0→1) y el exit (1→0). Mientras el
  // modal está lógicamente abierto shouldDraw==true; tras cerrarlo (*open=false) sigue
  // siendo true hasta que la transición de salida termina, así el modal + su scrim se
  // desvanecen en vez de desaparecer de golpe. El factor `presT` escala la opacidad de
  // todo el overlay (PushOpacity en Begin, PopOpacity en EndModal) y el alpha del scrim.
  // Se mantiene "modal" (captura input) hasta desvanecerse del todo; durante la salida
  // *open ya es false, así que los writes de cerrar/Escape/X son no-ops inofensivos.
  bool logicallyOpen = (open && *open);
  PresenceResult pres = BeginPresence(ctx, modalId, logicallyOpen);
  if (!pres.shouldDraw) {
    state.open = false;
  ...
  // brief 35 Part E: el scrim hace fade con el mismo factor de presence que la card.
  Color overlayColor(0.0f, 0.0f, 0.0f, 0.5f * pres.t);
  ctx->renderer.DrawRectFilled(Vec2(0, 0), viewport, overlayColor, 0.0f);

  // Forzar que el backdrop se dibuje antes que el modal
  ctx->renderer.FlushBatch();

  // brief 35 Part E: a partir de aquí todo el modal (card, título, borde, botón X y
  // el contenido) se dibuja con la opacidad de presence. Balanceado por PopOpacity en
  // EndModal (que solo se ejecuta cuando BeginModal devolvió true, contrato B2).
...
void EndModal() {
  ...
  // brief 35 Part E: cerrar el scope de opacidad de presence abierto en BeginModal
  // (PushOpacity(pres.t)). EndModal solo se ejecuta cuando BeginModal devolvió true,
  // que es exactamente cuando hizo el push → siempre balanceado.
...
DialogResult ContentDialog(const std::string &id, bool *open,
  ...
  // `*open == false` NO significa "no hacer nada": hay que seguir llamando a
  // BeginModal, porque es él quien conduce la transición de salida y, al terminarla,
  // libera activeModalId — el bloqueo GLOBAL de input del modal. Con el return
  // inmediato de antes ese frame no llegaba nunca y el bloqueo quedaba pegado:
  // input muerto en toda la app (con las animaciones corriendo) detrás de un
  // diálogo invisible. BeginModal ya sale barato cuando no hay nada que dibujar.
  const bool interactive = *open;

  bool escPressed = interactive && ctx->input.IsKeyPressed(UIKey::Escape);
  bool enterPressed = interactive &&
  ...
  if (interactive && !focusInit && !primaryText.empty()) {
  ...
  if (BeginModal(id, title, open, dlgSize)) {
    ...
    if (!primaryText.empty()) {
      ...
      // Durante el fade de salida los botones se siguen dibujando (para que se vean
      // desvanecerse) pero quedan inertes: `interactive` descarta sus clicks.
      if (Button(primaryText) && interactive)
        wantPrimary = true;
      PopButtonStyle();
    }
    if (!secondaryText.empty() && Button(secondaryText) && interactive)
      wantSecondary = true;
    if (!closeText.empty() && Button(closeText) && interactive)
      wantClose = true;
    EndHorizontal();
    EndModal();
  }
  ...
  // Solo mientras es interactivo: durante la salida el foco vuelve a la app.
  if (interactive && focusStart > 0 &&
      focusStart <= ctx->focusableWidgets.size()) {
  ...
  // Ya cerrado: este frame solo servía para dejar avanzar/terminar la salida y
  // liberar el bloqueo de input. No se emite resultado (el que cerró el diálogo ya
  // lo recibió en su momento) ni se repite el registro de accesibilidad.
  if (!interactive)
    return DialogResult::None;
