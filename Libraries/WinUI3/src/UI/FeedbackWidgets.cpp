...
void ProgressRing(const std::string& id, float size, float progress) {
  ...
  // brief 35 Part G: en modo determinado, el progreso mostrado se interpola hacia el
  // valor nuevo con springFloat[0] (motor unificado; reduceMotion→snap vía MotionConfig)
  // en vez de saltar. Se actualiza fuera del culling para no dar un brinco al volver a
  // ser visible. El modo indeterminado (progress<0) ya gira; no se toca. La a11y sigue
  // reportando el valor objetivo (progress), no el animado.
  float drawProgress = progress;
  if (progress >= 0.0f) {
    auto &pSpring = ctx->GetWidgetState(wid).springFloat[0];
    float tgt = std::clamp(progress, 0.0f, 1.0f);
    if (!pSpring.IsInitialized()) {
      pSpring.Configure(MotionTokens::Navigational, 1.0f);
      pSpring.SetImmediate(tgt);
    }
    pSpring.SetTarget(tgt);
    if (pSpring.IsAnimating()) ctx->NotifySpringFloatActive(wid);
    drawProgress = std::clamp(pSpring.Get(), 0.0f, 1.0f);
  }
  ...
  if (IsRectInViewport(ctx, pos, Vec2(diam, diam))) {
    ...
    if (progress >= 0.0f) {
      ...
      // brief 35 Part G: dibuja el valor interpolado (drawProgress), no el objetivo.
      float p = drawProgress;
...
bool Badge(int count, bool dot, std::optional<Vec2> anchorTopRight) {
  ...
  // brief 35 Part G: "pop" de escala al INCREMENTAR el contador. En la PRIMERA
  // aparición no hace pop. springFloat[0]=escala (target 1, nudged hacia arriba al
  // subir el contador); crítico → sin undershoot. reduceMotion → snap (sin pop).
  //
  // El estado se keyea por nº de secuencia (anonWidgetSeq), NO por lastGeneratedId:
  // los widgets a los que el badge se ancla (IconLabel, Label…) no llaman a
  // GenerateId, así que lastGeneratedId era el MISMO para varios badges seguidos y
  // los tres badges de una fila compartían un único estado. Con counts distintos
  // (7, 128, 0) cada uno leía el intVal que había dejado el anterior, disparaba
  // Nudge() en CADA frame y el spring nunca decaía: la píldora crecía hasta ocupar
  // media pantalla. El seed del idStack se mezcla, así que PushID(i) sigue
  // discriminando badges dentro de listas.
  float popScale = 1.0f;
  {
    // Mezcla djb2 (igual que PushID(int)); no se usa GenerateId para no sobrescribir
    // ctx->lastGeneratedId, que el Tooltip consulta para detectar cambio de widget.
    uint32_t seqId = ctx->CurrentIdSeed();
    seqId = ((seqId << 5) + seqId) + 0xBADEu; // discriminante "badge"
    seqId = ((seqId << 5) + seqId) + ctx->anonWidgetSeq++;
    uint32_t badgeStateId = AnimSlot(seqId, 5);
    auto &bs = ctx->GetWidgetState(badgeStateId);
    auto &popS = bs.springFloat[0];
    if (!popS.IsInitialized()) {
      popS.Configure(MotionTokens::Emphasis, 1.0f);
      popS.SetImmediate(1.0f);
      bs.intVal = count;
    } else {
      if (count > bs.intVal) popS.Nudge(0.35f);
      bs.intVal = count;
    }
    popS.SetTarget(1.0f);
    if (popS.IsAnimating()) ctx->NotifySpringFloatActive(badgeStateId);
    // Techo duro: el pop de diseño es 1→1.35 y vuelve. El clamp garantiza que ningún
    // estado corrupto (spring compartido, nudges acumulados, dt patológico) pueda
    // convertir la píldora en un blob gigante — a lo peor se pierde la animación.
    popScale = std::clamp(popS.Get(), 1.0f, 1.4f);
  }
  ...
  // brief 35 Part G: el fondo de la píldora escala con el pop alrededor de la esquina
  // (popScale ≥ 1 → solo crece; el texto se mantiene centrado en su tamaño base).
  float pw = w * popScale, ph = h * popScale;
  Vec2 pbp(corner.x - pw * 0.5f, corner.y - ph * 0.5f);
  ctx->renderer.DrawRectFilled(pbp, Vec2(pw, ph), accent, ph * 0.5f);
