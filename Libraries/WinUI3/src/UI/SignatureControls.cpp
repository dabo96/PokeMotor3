...
bool ToggleSwitch(const std::string &label, bool *value, const std::string &onText,
  ...
  // brief 35 Part C: posición del thumb (0=off, 1=on) por springFloat[0] — motor
  // unificado, interrumpible y con reduceMotion (SetTarget degrada a snap vía
  // MotionConfig). Sustituye el ApproachAnim manual, que no pasaba por MotionConfig.
  // La primera vez SetImmediate al estado correcto (sin animación fantasma inicial).
  auto &thumbSpring = ctx->GetWidgetState(id).springFloat[0];
  float target = current ? 1.0f : 0.0f;
  if (!thumbSpring.IsInitialized()) {
    thumbSpring.Configure(MotionTokens::Interactive, 1.0f);
    thumbSpring.SetImmediate(target);
  }
  thumbSpring.SetTarget(target);
  if (thumbSpring.IsAnimating()) ctx->NotifySpringFloatActive(id);
  float anim = std::clamp(thumbSpring.Get(), 0.0f, 1.0f);
...
int SplitButton(const std::string &label, uint32_t icon,
  ...
  // Si una constraint (fill / minWidth…) cambió el ancho, la zona primaria absorbe
  // la diferencia y el chevron conserva su ancho fijo. Sin esto el chevron se
  // quedaba en la posición del ancho medido y dejaba un hueco a su derecha.
  primaryW = std::max(S(24.0f), finalSize.x - chevW);
  ...
  if (IsRectInViewport(ctx, widgetPos, finalSize)) {
    ...
    // Fondo: UNA sola superficie redondeada para todo el control. Antes se pintaba
    // un rect redondeado por zona, así que las esquinas interiores también se
    // redondeaban y dejaban un hueco en forma de lente en el centro → el control
    // se leía como dos botones separados. Ahora el estado hover/pressed de la zona
    // activa se pinta ENCIMA recortado a esa zona, de modo que el redondeo solo
    // aparece en las esquinas exteriores.
    ctx->renderer.DrawRectFilled(widgetPos, finalSize, bs.background.normal,
                                 bs.cornerRadius);
    if (primHover || chevHover) {
      Vec2 zonePos = primHover ? primPos : chevPos;
      Vec2 zoneSize = primHover ? primSize : chevSize;
      // FlushBatch antes de Push/PopClipRect: sin él el clip se aplicaría
      // retroactivamente al fondo ya encolado (patrón conocido en cards/splitter).
      ctx->renderer.FlushBatch();
      ctx->renderer.PushClipRect(zonePos, zoneSize);
      ctx->renderer.DrawRectFilled(widgetPos, finalSize,
                                   stateColor(bs.background, true),
                                   bs.cornerRadius);
      ctx->renderer.FlushBatch();
      ctx->renderer.PopClipRect();
    }
  ...
  int result = 0;
  // El cierre por click-fuera de BeginFlyout excluye el anchorRect (== este botón),
  // así que reclicar el chevron reabría el menú de inmediato y nunca se cerraba.
  // Toggle explícito, igual que CommandBar "···" / BreadcrumbBar "...".
  if (IsFlyoutOpen(menuId)) {
    // Con el menú abierto, un clic en cualquier zona del botón solo lo descarta
    // (light dismiss): no dispara onPrimary ni lo reabre.
    if (primClicked || chevClicked)
      CloseFlyout(menuId);
  } else {
    if (primClicked) {
      if (onPrimary)
...
void DropDownButton(const std::string &label, uint32_t icon,
  ...
  // Mismo toggle que SplitButton: BeginFlyout excluye el anchorRect del cierre por
  // click-fuera, así que sin esto reclicar el botón reabría el menú al instante.
  if (clicked) {
    if (IsFlyoutOpen(menuId))
      CloseFlyout(menuId);
    else
