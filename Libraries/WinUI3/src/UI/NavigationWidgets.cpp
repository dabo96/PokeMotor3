...
std::string NavigationView(const std::string& id,
  ...
  // brief 35 Part D: Y (top) objetivo del ROW seleccionado, capturada en el recorrido.
  // El "pill" de selección (fondo + barra de acento) se dibuja ANTES de las filas (bajo
  // el texto) en el valor suavizado del spring, y el objetivo se fija tras la lista.
  // rowH es constante → basta animar la Y.
  float indTargetY = 0.0f;
  bool indFound = false;
  ...
  drawList = [&](const std::vector<NavItem>& list, int depth, float& yy) {
    for (const NavItem& it : list) {
      ...
      if (selected) {
        // brief 35 Part D: ni el fondo de selección ni la barra se pintan aquí; el pill
        // (fondo + barra) se dibuja deslizando ANTES de la lista para quedar bajo el
        // texto. Aquí solo se registra la Y (top) del row seleccionado.
        indFound = true;
        indTargetY = rp.y;
      } else if (hover) {
        Color hv = ctx->style.panel.headerBackground;
      ...
      if (it.badge > 0) {
        if (showLabels) {
          ...
          float bfs = fontSize * 0.82f;
          Vec2 ts = MeasureTextCached(ctx, bt, bfs);
          float bh = fontSize + S(2.0f);
          // Pastilla nunca más estrecha que alta: con un dígito queda círculo perfecto.
          float bw = std::max(ts.x + S(10.0f), bh);
          Vec2 bpos(rightEdge - bw, rp.y + (rs.y - bh) * 0.5f);
          ctx->renderer.DrawRectFilled(bpos, Vec2(bw, bh), ctx->style.accentColor,
                                       bh * 0.5f);
          // Centrado con la caja medida real (ts), igual que el widget Badge.
          ctx->renderer.DrawText(
              Vec2(bpos.x + (bw - ts.x) * 0.5f, bpos.y + (bh - ts.y) * 0.5f), bt,
              Color(1, 1, 1, 1), bfs);
        } else {
  ...
  // brief 35 Part D: "pill" de selección deslizante (fondo + barra de acento) dibujado
  // ANTES de las filas para quedar BAJO el texto/icono del ítem, usando el valor
  // suavizado del spring del CONTENEDOR (el objetivo se fija tras recorrer la lista).
  // En el primer frame el spring aún no existe → no se dibuja; se inicializa abajo y
  // aparece ya colocado al frame siguiente.
  uint32_t navIndId = GenerateId("NAVINDICATOR:", id.c_str());
  {
    auto indIt = ctx->widgetStates.find(navIndId);
    if (indIt != ctx->widgetStates.end() &&
        indIt->second.springFloat[0].IsInitialized()) {
      float py = indIt->second.springFloat[0].Get();
      ctx->renderer.DrawRectFilled(Vec2(barPos.x, py), Vec2(curW, rowH),
                                   ctx->style.panel.headerBackground, S(4.0f));
      ctx->renderer.DrawRectFilled(Vec2(barPos.x + S(3.0f), py + rowH * 0.25f),
                                   Vec2(S(3.0f), rowH * 0.5f),
  ...
  // brief 35 Part D: fijar el objetivo del pill tras conocer el ítem seleccionado (el
  // DIBUJO ya se hizo antes de la lista con el valor suavizado). Al cambiar de selección
  // la Y viaja desde el ítem anterior en vez de saltar. Primer frame SetImmediate (no
  // anima desde el borde). Token Navigational; reduceMotion → snap.
  if (indFound) {
    auto &yS = ctx->GetWidgetState(navIndId).springFloat[0];
    if (!yS.IsInitialized()) {
      yS.Configure(MotionTokens::Navigational, 1.0f);
      yS.SetImmediate(indTargetY);
    }
    yS.SetTarget(indTargetY);
    if (yS.IsAnimating())
      ctx->NotifySpringFloatActive(navIndId);
  }
...
bool DrawCommandButton(UIContext* ctx, const Vec2& pos, const Vec2& size,
  ...
  if (hover) {
    // Overlay tintado por tema: visible sobre CUALQUIER fondo (incl. una franja
    // pintada con panel.headerBackground, donde un fill de ese mismo color sería
    // invisible). Mismo criterio Fluent que SegmentedControl. Pressed más marcado.
    Color hv = ctx->style.isDarkTheme
                 ? Color(1.0f, 1.0f, 1.0f, pressed ? 0.14f : 0.08f)
                 : Color(0.0f, 0.0f, 0.0f, pressed ? 0.10f : 0.06f);
...
void CommandBar(const std::string& id, const std::vector<CommandItem>& primary,
  ...
  if (needOverflow) {
    ...
    if (hover) {
      bool ofPressed = ctx->input.IsMouseDown(0);
      Color ohv = ctx->style.isDarkTheme
                    ? Color(1.0f, 1.0f, 1.0f, ofPressed ? 0.14f : 0.08f)
                    : Color(0.0f, 0.0f, 0.0f, ofPressed ? 0.10f : 0.06f);
      ctx->renderer.DrawRectFilled(ob, os, ohv, S(4.0f));
    }
    ...
    // Toggle: abrir con el primer clic, cerrar con el segundo. Antes siempre se
    // llamaba OpenFlyout, pero el cierre por click-fuera de BeginFlyout excluye el
    // anchorRect (== este botón "···"), así que reclicar el botón lo reabría de
    // inmediato → el menú nunca se cerraba desde su propio botón.
    if (hover && ctx->input.IsMousePressed(0)) {
      if (IsFlyoutOpen(ofId))
        CloseFlyout(ofId);
      else
        OpenFlyout(ofId);
    }
...
int BreadcrumbBar(const std::string& id, const std::vector<std::string>& crumbs) {
  ...
  if (total <= availW || n <= 2) {
    ...
    // Toggle (mismo motivo que el "···" de CommandBar): reclicar el botón "..."
    // lo reabría porque BeginFlyout excluye el anchorRect del cierre por click-fuera.
