...
namespace FluentUI {
    ...
    void Renderer::BeginFrame(const Color& clearColor) {
        // Defer backend->BeginFrame to EndFrame: in Vulkan shared mode the
        // backend records onto the engine's command buffer, which only exists
        // once we're inside the engine's UI pass (i.e. at EndFrame time, when
        // the host has called SetFrameCommandBuffer). Calling it here would run
        // it with no/stale command buffer. We just capture the clear color.
    ...
    void Renderer::EndFrame() {
        // Begin the backend frame now — by this point the host (engine) has
        // supplied the command buffer for shared mode, so BeginFrame can safely
        // record viewport/scissor. Paired with the EndFrame() call below.
        if (backend) backend->BeginFrame(pendingClearColor);
