...
namespace FluentUI {
    ...
    void Renderer::BeginFrame(const Color& clearColor) {
        ...
        currentTextParams = TextRenderParams{};

        // Brief 35-A: the window background is the default backdrop for the text
        // contrast direction; widgets that fill their own surface push theirs.
        frameClearColor_ = clearColor;
        textBackdropStack_.clear();
        // Brief 35-C: grid fitting is a per-scope opt-out; it must not leak frames.
        textGridFitScope_ = true;
            ...
            for (int i = 0; i < (int)RenderLayer::Count; ++i) {
                for (const auto& batch : layerBatches[i]) {
                    ...
                    if (batch.isAcrylic) {
                        ...
                        // Brief 35-A/35-B: text params travel as backend STATE, set
                        // right before the batch that needs them, so the DrawBatch
                        // signature (and every backend overriding it) stays intact.
                        backend->SetTextRenderParams(batch.textParams);
    ...
    // Brief 35-B: subpixel coverage is only meaningful when the fragment blends
    // straight against the final, opaque surface with an axis-aligned, uniformly
    // scaled transform. This Renderer has no transform stack (text is emitted in
    // absolute device px and never rotated or sheared), so those two conditions of
    // the brief hold by construction; what is left is capability + preference +
    // "are we drawing to the screen".
    bool Renderer::CanUseSubpixel() const {
        return textQuality_.subpixelEnabled
            && !renderingToOffscreen_
            && backend != nullptr
            && backend->Supports(RenderCap::SubpixelText);
    }

    // Brief 35-A: pick the transfer curve for this run. The direction is decided by
    // relative luminance of text vs the surface it sits on — NOT by a global theme
    // flag: a dark label on a light card inside a dark window still needs the
    // dark-on-light curve.
    TextRenderParams Renderer::ComputeTextParams(const Color& textColor) const {
        TextRenderParams p;
        if (textQuality_.contrastEnabled) {
            auto luminance = [](const Color& c) {
                // Rec. 709 luma on the sRGB values. Exact linearization isn't worth
                // it here — we only need to know which side of the contrast we're on.
                return 0.2126f * c.r + 0.7152f * c.g + 0.0722f * c.b;
            };
            const Color& bg = CurrentTextBackdrop();
            const bool lightOnDark = luminance(textColor) > luminance(bg);
            p.gamma = lightOnDark ? textQuality_.gammaLightOnDark
                                  : textQuality_.gammaDarkOnLight;
            p.contrast = textQuality_.contrast;
        }
        if (CanUseSubpixel()) {
            p.subpixel = textQuality_.bgrStripes ? -1 : 1;
            p.fringe = textQuality_.fringe;
        }
        return p;
    }

    static bool SameTextParams(const TextRenderParams& a, const TextRenderParams& b) {
        return a.gamma == b.gamma && a.contrast == b.contrast &&
               a.subpixel == b.subpixel && a.fringe == b.fringe;
    }

    void Renderer::EnsureBatchState(ShaderType shader, void* texture, Color color,
                                    const TextRenderParams* textParams) {
        ...
        // Brief 35-A: identity for anything that isn't MSDF text.
        const TextRenderParams wantTextParams = textParams ? *textParams : TextRenderParams{};
        bool changed = currentBatchShader != shader || currentBatchTexture != texture ||
                       ...
                       !SameTextParams(currentTextParams, wantTextParams);
        ...
        if (changed && (!quadVertices.empty() || !sdfInstances.empty() || !lineVertices.empty())) {
        ...
        currentTextParams = wantTextParams;
        // Brief 29 Part B: only the MSDF text path carries a real pxRange. Any
        // other shader clears it (the outgoing MSDF batch was already flushed
        // above with its range intact); the MSDF DrawText path re-sets it after
        // this call from FontMSDF::GetPixelRange().
        if (shader != ShaderType::MSDF) currentBatchPxRange = 0.0f;
    }
        ...
        if (!quadVertices.empty()) {
            ...
            if (!batches.empty()) {
                ...
                if (!last.isLines && !last.isSDF && !last.isAcrylic && last.type == type && last.texture == texture &&
                    ...
                    last.msdfPxRange == currentBatchPxRange &&
                    SameTextParams(last.textParams, currentTextParams)) {
            ...
            if (!merged) {
                ...
                batch.msdfPxRange = currentBatchPxRange; // Brief 29 Part B
                batch.textParams = currentTextParams;    // Brief 35-A/35-B
    ...
    void Renderer::DrawRect(const Vec2& pos, const Vec2& size, const Color& color, float cornerRadius, const Color& borderBottom) {
        ...
        // Brief 34 Parte E: color inferior del bisel. (0,0,0,0) por defecto ⇒ el shader
        // lo detecta como borde plano (retrocompatible con todos los call-sites).
        s.borderR2 = borderBottom.r; s.borderG2 = borderBottom.g; s.borderB2 = borderBottom.b;
    ...
    void Renderer::DrawText(const Vec2& pos, const std::string& text, const Color& color, float fontSize) {
        ...
        // Brief 29 Part A: small text is served by hinted per-size bitmap buckets so
        // every stem is grid-fit and the weight is uniform regardless of subpixel
        // position. Path chosen by the shared PickTextPath (so MeasureText agrees).
        // Glyphs draw 1:1 — quad = bitmap size, no scaling — at pixel-snapped
        // positions with a rounded pen (Part D). That 1:1 is the whole point.
        FontSystem::TextPath tp = fontSystem_.PickTextPath(fontSize);

        if (tp.bitmap) {
            void* atlas = fontSystem_.BitmapAtlasTexture();
            if (atlas) {
                EnsureBatchState(ShaderType::Text, atlas, color);
                float baseline = std::round(pos.y + fontSystem_.BucketAscent(tp.sizePx));
                float lineH = fontSystem_.BucketLineHeight(tp.sizePx);
                float penX = pos.x;
                const char* ptr = text.data(); const char* end = ptr + text.size();
                while (ptr < end) {
                    uint32_t cp = DecodeUTF8(ptr, end);
                    if (cp == 0) break;
                    if (cp == '\n') { penX = pos.x; baseline = std::round(baseline + lineH); continue; }
                    const FontSystem::Glyph* g = fontSystem_.GetGlyphBucket(tp.sizePx, cp);
                    if (!g) g = fontSystem_.GetGlyphBucket(tp.sizePx, '?');
                    if (g) {
                        float x0 = std::round(penX + g->bearing.x);
                        float y0 = std::round(baseline - g->bearing.y);
                        float w = g->size.x; float h = g->size.y; // integer bitmap, 1:1
                        if (w > 0 && h > 0) {
                            if (quadVertices.size() + 4 > MAX_QUAD_VERTICES - 4) FlushBatch(ShaderType::Text, atlas, color);
                            unsigned int base = static_cast<unsigned int>(quadVertices.size());
                            // FreeType bitmaps are top-down: the glyph's top row lives
                            // at uv0.y, so the top vertices sample uv0.y (same Y
                            // convention as the MSDF path). The dead bitmap-fallback
                            // path below uses the opposite mapping and renders flipped.
                        ...
                        penX = std::round(penX + g->advance); // grid-fit expects integer pen
                    }
                }
                return;
            }
        ...
        // Brief 35-D: pick the optical instance for this size (falls back to the
        // single text atlas when no display atlas is shipped). MeasureText resolves
        // the same way, so layout can't desync from the pixels.
        FontMSDF* mf = fontSystem_.MSDFForSize(fontSize); // owner's atlas in multi-window
        if (mf && mf->IsLoaded()) {
            // Brief 35-A/35-B: the curve direction depends on the backdrop, so it is
            // part of the batch identity — computed BEFORE EnsureBatchState so a
            // change flushes the previous run instead of retinting it.
            const TextRenderParams tparams = ComputeTextParams(color);
            // Issue 1: Only flush if shader/texture/color/params state actually changes
            EnsureBatchState(ShaderType::MSDF, mf->GetTextureHandle(), color, &tparams);
            // Brief 29 Part B: carry the atlas' real distanceRange to the shader.
            currentBatchPxRange = mf->GetPixelRange();

            float scale = fontSize / mf->GetEmSize();

            // Brief 35-C: vertical grid fitting, as a translation of the WHOLE run.
            //   1. the baseline goes to a whole pixel (kills inter-frame shimmer and
            //      makes every horizontal feature of the run consistent), and
            //   2. δ shifts the run so the baseline / x-height / cap-height trio
            //      lands as close to the grid as one shared offset allows.
            // Gated off while the run is animating position (§4): snapping a moving
            // label produces sustained judder, far worse than a one-frame settle pop.
            const bool gridFit = textQuality_.verticalGridFit && textGridFitScope_;
            const float delta = gridFit ? fontSystem_.VerticalPhase(fontSize) : 0.0f;
            float ascentPx = (mf->GetAscender() > 0.0f) ? mf->GetAscender() * fontSize
                                                        : fontSize * 0.8f;
            const float lineStep = fontSize * mf->GetLineHeight();
            int lineIndex = 0;
            // Each line snaps from the UNSHIFTED position and then takes δ, so the
            // phase never accumulates down a multi-line block.
            auto baselineFor = [&](int line) {
                const float raw = pos.y + ascentPx + static_cast<float>(line) * lineStep;
                return gridFit ? std::round(raw) + delta : raw;
            };
            float baseline = baselineFor(0);

            // Brief 35-B / invariant 2: the run's ORIGIN is snapped once; every glyph
            // then sits at its exact fractional offset from it. Rounding each glyph's
            // x independently is what made the two stems of "tt" come out at unequal
            // widths — and it would also throw away the subpixel path's whole point,
            // since three-tap sampling only pays off at fractional positions.
            const float originX = gridFit ? std::round(pos.x) : pos.x;
            float xpos = originX;
            const char* ptr = text.data(); const char* end = ptr + text.size();
            ...
            while (ptr < end) {
                uint32_t cp = DecodeUTF8(ptr, end);
                if (cp == 0) break;
                if (cp == '\n') { xpos = originX; baseline = baselineFor(++lineIndex); continue; }
                const FontMSDF::Glyph* g = mf->GetGlyph(cp);

                if (g) {
                    // Both coordinates now derive from the run's snapped origin
                    // WITHOUT per-glyph rounding. The MSDF shader AA absorbs the
                    // fractional edges; w/h and advance stay fractional.
                    float x0 = xpos + g->bearing.x * scale; float y0 = baseline - g->bearing.y * scale;
        ...
        if (fontSystem_.IsFontLoaded() && bitmapAtlas) {
            ...
            while (ptr < end) {
                ...
                if (g) {
                    float x0 = std::round(penX + g->bearing.x * scale); float y0 = std::round(baseline - g->bearing.y * scale); // Brief 29 Part D
                    ...
                    if (w > 0 && h > 0) {
                        ...
                        unsigned int base = static_cast<unsigned int>(quadVertices.size());
                        // Brief 29: correct the Y flip — FreeType bitmaps are top-down,
                        // so the top vertices sample uv0.y (matches the MSDF & bucket
                        // paths). This path was rendering upside-down before.
                        quadVertices.push_back({x0,y0,1,1,1,1,g->uv0.x,g->uv0.y});
                        quadVertices.push_back({x0+w,y0,1,1,1,1,g->uv1.x,g->uv0.y});
                        quadVertices.push_back({x0+w,y0+h,1,1,1,1,g->uv1.x,g->uv1.y});
                        quadVertices.push_back({x0,y0+h,1,1,1,1,g->uv0.x,g->uv1.y});
    ...
    float Renderer::TextTopForVCenter(float containerTop, float containerHeight, float fontSize) {
        // LineAdvancePx is the same line box MeasureText reports (and it already
        // picks the bitmap-bucket vs MSDF path the way DrawText will), so callers
        // can't drift from what actually gets drawn.
        const float lineBox = fontSystem_.LineAdvancePx(fontSize);
        return containerTop + (containerHeight - lineBox) * 0.5f;
    }
    ...
    void Renderer::DrawIconGlyphVCentered(float x, float centerY, uint32_t codepoint, const Color& color, float fontSize) {
        if (!fontSystem_.IsIconFontLoaded()) return;
        const FontSystem::Glyph* g = fontSystem_.GetIconGlyph(codepoint);
        if (!g) return;
        ...
        float w = g->size.x * scale;
        float h = g->size.y * scale;
        // Center the glyph's real visual box on centerY, ignoring the icon font's
        // ascent so it aligns optically with text centered on the same line.
        float x0 = std::round(x + g->bearing.x * scale);
        float y0 = std::round(centerY - h * 0.5f);
    ...
    void Renderer::PushClipRect(const Vec2& pos, const Vec2& size) {
        ...
        if (!quadVertices.empty() || !lineVertices.empty() || !sdfInstances.empty()) FlushBatch();
        clipStack.push_back(rect);
        if (backend) backend->PushClipRect(rect.x, rect.y, rect.width, rect.height);
    }
