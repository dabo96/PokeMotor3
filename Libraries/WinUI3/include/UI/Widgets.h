...
/// dialog's controls. When it returns != None it sets `*open = false`. Role: Dialog.
/// Call it EVERY frame, NOT wrapped in `if (open)`: it draws nothing while closed,
/// but it needs the frames right after a close to run the exit fade and release the
/// modal's global input capture. Skipping those frames leaves the whole UI blocked
/// behind an invisible dialog (the core has a safety net for it, but the exit
/// animation is lost).
...
// brief 32: widget Card (data-driven) + one-shots. Self-contained (solo depende de
// Vec2/string/functional); se apoya en el material/elevación resueltos en el .cpp.
#include "UI/CardWidget.h"

// brief 31: capa de fachada (callback + scope-guard RAII) sobre los contenedores
// Begin*/End*. Va al FINAL: necesita que todas las declaraciones Begin*/End* y
// los tipos (TableColumn, FlyoutPlacement, Rect…) ya estén visibles.
