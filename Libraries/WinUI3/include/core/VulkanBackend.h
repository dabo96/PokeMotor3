...
private:
    ...
    // 112 bytes — fits the 128-byte guaranteed push-constant range.
    ...
    // text[4] (brief 35-A/35-B) = {gamma, contrast, subpixel, fringe}; msdf.frag and
    // msdf_subpixel.frag declare them as 4 trailing scalars at offsets 96..108.
    ...
    static_assert(sizeof(PushConstants) == 112, "push constants must stay <= 128 bytes");
    ...
    // Brief 35-B: true only when WE created the device and enabled dualSrcBlend.
    // In engine-shared mode we cannot know whether the host enabled the feature,
    // and using it without it being enabled is undefined — so we stay grayscale.
    bool             dualSrcBlendSupported = false;
    // Params for the text batches drawn from now on (brief 35-A/35-B).
    ...
    // Brief 35-B: dual-source (ClearType) variant. Only created when the device
    // has the dualSrcBlend feature ENABLED — a shader declaring an output with
    // Index 1 is illegal otherwise, so this must stay null on such devices.
    VkShaderModule msdfSubpixelFrag = VK_NULL_HANDLE;
    ...
    // Brief 35-B: same vertex input + layout as pipeMSDF, but blending from SRC1
    // (dual-source). Null when dualSrcBlend isn't available/enabled.
    VkPipeline pipeMSDFSubpixel = VK_NULL_HANDLE;
    ...
    // dualSource (brief 35-B): blend from SRC1_COLOR/SRC1_ALPHA instead of the
    // fragment's own alpha — required by the subpixel text pipeline.
