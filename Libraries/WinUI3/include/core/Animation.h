...
namespace FluentUI {
    ...
    // brief 35 Part H: motion tokens — the single source of truth for animation
    // "response" (≈ settle time, s) across widgets. Changing a token here retimes the
    // whole family that uses it; no loose literals in widgets. These feed
    // SpringValue::Configure(response) and the presence enter/exit responses.
    // Interactive is the value the Button pilot used (0.18s); the rest are graded
    // slower for larger, more deliberate motion.
    namespace MotionTokens {
        inline constexpr float Interactive  = 0.18f;  // hover/press color feedback
        inline constexpr float Navigational = 0.28f;  // sliding indicator, page change
        inline constexpr float Expand       = 0.32f;  // Expander height, NavView width
        inline constexpr float Emphasis     = 0.45f;  // attention, emphasized entrances
    }
