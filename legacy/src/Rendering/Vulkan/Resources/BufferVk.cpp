...
bool BufferVk::create(VmaAllocator allocator, VkDeviceSize size,
    ...
    if (vmaCreateBuffer(allocator, &bufInfo, &allocInfo,
        ...
        std::fprintf(stderr, "[Vulkan] vmaCreateBuffer failed (size=%llu)\n",
...
bool BufferVk::CreateStorage(VmaAllocator allocator, VkDeviceSize size,
                             VkBufferUsageFlags extraUsage) {
    return create(allocator, size,
...
void BufferVk::CmdFill(VkCommandBuffer cmd, uint32_t pattern,
                       VkDeviceSize size, VkDeviceSize offset) const {
    vkCmdFillBuffer(cmd, m_buffer, offset, size, pattern);
}
