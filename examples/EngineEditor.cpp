...
void BuildEditorUI(UIBuilder& ui, FluentApp& app, EditorState& state) {
    ...
    // brief 31: ScopedPanel — guard RAII que reenvía la firma completa de
    // BeginPanel (acrylic/pos) y cierra automáticamente. Corrige el bug previo
    // (EndPanel se llamaba SIEMPRE, fuera del if; contrato B2 → solo si abrió).
    {
        ScopedPanel p{"acrylic_demo", Vec2(300, 200), /*reserveLayoutSpace=*/false,
